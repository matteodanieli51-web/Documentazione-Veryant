...
Executes an HTTP request using PATCH method specifying the data stream and having the MIME type set automatically.

#### Format 1

```cobol
void doPatchEx( strUrl, content )
```

#### Format 2

```cobol
void doPatchEx( strUrl, type, content )
```

#### Format 3

```cobol
void doPatchEx( strUrl, type, content, hasDummyRoot )
```
...
1. In Format 1, *type* is set to "application/json" or "application/xml" depending on the Content-type request header field. If no Content-type field is available in the request header, then the type is controlled by the [iscobol.rest.default_stream](../../../is-cobol-evolve/SDK-Users-Guide/Compiler-and-Runtime/Configuration/Configuration-Properties#rest_defualt_stream) configuration setting.
...
Issue a PATCH request with parameters:
...
...
   ...
   01 request identified by "newdata".
      03 identified by "mail".
         05 mail pic x any length.
...
   procedure division.
...
   move "newaddress@mail.com" to mail.
   http:>doPatchEx("http://example.com/users/user1", request).
...
```
