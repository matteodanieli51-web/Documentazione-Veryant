...
Executes an HTTP request using DELETE method specifying the data stream and having the MIME type set automatically.

#### Format 1

```cobol
void doDeleteEx( strUrl, content )
```

#### Format 2

```cobol
void doDeleteEx( strUrl, type, content )
```

#### Format 3

```cobol
void doDeleteEx( strUrl, type, content, hasDummyRoot )
```
...
Issue a DELETE request with parameters:
...
...
   ...
   01 request identified by "request".
      03 identified by "reason".
         05 reason pic x any length.
...
   procedure division.
...
   move "Removing trash content" to reason.
   http:>doDeleteEx("http://example.com/trash/foo", request).
...
```
