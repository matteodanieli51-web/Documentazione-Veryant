...
async function persistToFileStreaming(db, format, filePath, runtime) {
...
async function streamJsonToFile(data, filePath, runtime) {
    if (runtime === 'node') {
...
function streamLargeJsonToFile(data, stream, resolve, reject) {
    try {
        stream.write('{');
        let isFirst = true;
...
async function streamBinaryToFile(data, filePath, runtime) {
    if (runtime === 'node') {
        const fs = await import('node:fs');
        const { createWriteStream } = fs;
        return new Promise((resolve, reject)=>{
...
async function restoreFromBinaryData(data, runtime) {
    ...
    const deserialized = decode(data);
