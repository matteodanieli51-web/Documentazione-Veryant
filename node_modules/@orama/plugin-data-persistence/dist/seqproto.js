...
function serializeNumberArray(ser, arr) {
    ser.serializeUInt32(arr.length);
    for(let i = 0; i < arr.length; i++){
 ...
 */ export function serializeOramaInstance(db) {
    const raw = save(db);
    const ser = createSer();
 ...
 */ export function cloneRawData(raw) {
    const ser = createSer();
    ser.serializeUInt32(1);
    serializeValue(ser, raw);
    const des = createDes(ser.getBuffer());
    des.deserializeUInt32(); // version
    return deserializeValue(des);
}

//# sourceMappingURL=seqproto.js.map